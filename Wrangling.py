...
df_wetter["regen_imputiert"] = df_wetter["regen"].isna()
...
df_wetter["maximale_windgeschwindigkeit_imputiert"] = df_wetter["maximale_windgeschwindigkeit"].isna()
verhaeltnis_wind = (df_wetter["maximale_windgeschwindigkeit"] / df_wetter["windgeschwindigkeit"]).mean()
df_wetter["maximale_windgeschwindigkeit"] = df_wetter["maximale_windgeschwindigkeit"].fillna(
    df_wetter["windgeschwindigkeit"] * verhaeltnis_wind
)

#Schnee hinzufügen, sobald Temperatur unter 2 Grad und Regen vorhanden (Schnee kann anscheinend schon ab 2 Grad fallen)
df_wetter["schnee_vorhanden"] = np.where(
    (df_wetter["temperatur"] < 2) & (df_wetter["regen"] > 0),
    1,
    0
)
df_wetter["schnee_intensität"] = np.where(
    (df_wetter["temperatur"] < 2) & (df_wetter["regen"] > 0),
    df_wetter["regen"],
    0
)
...
df_oil["7_day_average_oil_price"] = (
    df_oil["oil_price"]
    .rolling(window=7)
    .mean()
    .shift(1)
...
df = df[
    [
        ...
        "maximale_windgeschwindigkeit_imputiert",
        "temperatur",
        "oil_price",
        "oil_price_imputiert",
        "90_day_average_oil_price",
        "oil_trend",
        "oil_volatility_90",
        "schnee_vorhanden",
        "schnee_intensität"
...
schweiz_feiertag = holidays.Switzerland(
    subdiv="ZH",
...
df[zaehlspalten] = df[zaehlspalten].astype(int)
df["piste_10_binär"] = df["piste_10_binär"].astype(bool)
df["schnee_vorhanden"] = df["schnee_vorhanden"].astype(bool)
